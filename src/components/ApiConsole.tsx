...
const ApiConsole: React.FC = () => {
  ...
  const onDragMove = (e: MouseEvent) => {
    const newHeight = Math.max(MIN_HEIGHT, Math.min(MAX_HEIGHT, startHeight.current + (startY.current - e.clientY)));
    setHeight(newHeight);
  };
  const onDragEnd = () => {
  ...
  return (
    <div style={{
      ...
      minHeight: expanded ? MIN_HEIGHT : 36,
      maxHeight: expanded ? MAX_HEIGHT : 36,
      height: expanded ? height : 36,
      ...
      {expanded && (
        ...
        >
          <GripVertical size={16} color="#B0BEC5" />
        </div>
      )}
      <div style={{ display: 'flex', alignItems: 'center', padding: '0 16px', height: 36, borderBottom: expanded ? '1px solid #23272e' : 'none', background: '#23272e', userSelect: 'none', marginTop: expanded ? 8 : 0 }}>
        <button onClick={() => setExpanded(e => !e)} style={{ background: 'none', border: 'none', color: '#B0BEC5', cursor: 'pointer', marginRight: 8 }} title={expanded ? 'Ocultar consola' : 'Expandir consola'}>
      ...
      {expanded && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0 0 0', userSelect: 'text' }}>
          {logs.length === 0 ? (
            <div style={{ color: '#B0BEC5', textAlign: 'center', marginTop: 32 }}>No hay peticiones registradas aún.</div>
          ) : (
            logs.map(log => (
              <div key={log.id} style={{ margin: '0 0 10px 0', padding: '8px 20px', borderLeft: `3px solid ${methodColors[log.method] || '#B0BEC5'}`, background: '#20222b', borderRadius: 6, boxShadow: '0 1px 4px 0 rgba(0,0,0,0.04)' }}>
                ...
                {log.comment && <div style={{ color: '#B0BEC5', fontSize: 12, margin: '2px 0 4px 0' }}># {log.comment}</div>}
                {log.body && <div style={{ color: '#B0BEC5', fontSize: 12, margin: '2px 0 2px 0' }}>Body: <pre style={{ display: 'inline', color: '#ECEFF1' }}>{prettyJson(log.body)}</pre></div>}
                {log.response && <div style={{ color: '#B0BEC5', fontSize: 12, margin: '2px 0 2px 0' }}>Resp: <pre style={{ display: 'inline', color: '#ECEFF1' }}>{prettyJson(log.response)}</pre></div>}
